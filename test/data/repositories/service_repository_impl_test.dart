@GenerateNiceMocks([MockSpec<ServiceApiService>()])
...
import 'service_repository_impl_test.mocks.dart';

void main() {
  late ServiceRepositoryImpl repository;
  late MockServiceApiService mockApiService;

  setUp(() {
    mockApiService = MockServiceApiService();
    repository = ServiceRepositoryImpl(mockApiService);
  });
  ...
  group('getServices', () {
    test('should return list of GithubModel when API call is successful', () async {
      // arrange
      when(mockApiService.getService())
      ...
      // assert
      expect(result, [tGithubModel]);
      verify(mockApiService.getService());
    ...
    test('should throw Exception when API call fails', () async {
      // arrange
      when(mockApiService.getService())
          .thenThrow(Exception('Failed to fetch Service'));

      // act
      final call = repository.getServices;
