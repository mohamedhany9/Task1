...
List<GithubModel> githubModelFromJson(String str) => List<GithubModel>.from(json.decode(str).map((x) => GithubModel.fromJson(x)));

String githubModelToJson(List<GithubModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
final defaultBranchValues = EnumValues({
  ...
  "travis-coveralls": DefaultBranch.TRAVIS_COVERALLS
});
...
enum EventsUrl {
  HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_EVENTS_PRIVACY
}

final eventsUrlValues = EnumValues({
  "https://api.github.com/users/mralexgray/events{/privacy}": EventsUrl.HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_EVENTS_PRIVACY
});

enum FollowingUrl {
  HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_FOLLOWING_OTHER_USER
}

final followingUrlValues = EnumValues({
  "https://api.github.com/users/mralexgray/following{/other_user}": FollowingUrl.HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_FOLLOWING_OTHER_USER
});

enum GistsUrl {
  HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_GISTS_GIST_ID
}

final gistsUrlValues = EnumValues({
  "https://api.github.com/users/mralexgray/gists{/gist_id}": GistsUrl.HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_GISTS_GIST_ID
});
...
final loginValues = EnumValues({
  "mralexgray": Login.MRALEXGRAY
});

enum NodeId {
  MDQ6_VX_NLCJ_I2_MJ_UX_NW
}

final nodeIdValues = EnumValues({
  "MDQ6VXNlcjI2MjUxNw==": NodeId.MDQ6_VX_NLCJ_I2_MJ_UX_NW
});

enum StarredUrl {
  HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_STARRED_OWNER_REPO
}

final starredUrlValues = EnumValues({
  "https://api.github.com/users/mralexgray/starred{/owner}{/repo}": StarredUrl.HTTPS_API_GITHUB_COM_USERS_MRALEXGRAY_STARRED_OWNER_REPO
});
